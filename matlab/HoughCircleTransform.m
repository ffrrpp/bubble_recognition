function [accum, varargout] = HoughCircleTransform(img, radrange, varargin)
%Detect circular shapes in a grayscale image. Resolve their center
%positions and radii.
%
%  [accum, circen, cirrad] = HoughCircleTransform(
...
%     tic;
%     [accum, circen] = HoughCircleTransform(rawimg, ...
...
if ndims(img) ~= 2 || ~isnumeric(img),
    error('HoughCircleTransform: ''img'' has to be 2 dimensional');
end
if ~all(size(img) >= 32),
    error('HoughCircleTransform: ''img'' has to be larger than 32-by-32');
end

if numel(radrange) ~= 2 || ~isnumeric(radrange),
    error(['HoughCircleTransform: ''radrange'' has to be ', ...
...
if nargin > (1 + vap_grdthres),
    if isnumeric(varargin{vap_grdthres}) && ...
        ...
        error(['HoughCircleTransform: ''grdthres'' has to be ', ...
...
if nargin > (1 + vap_fltr4LM),
    if isnumeric(varargin{vap_fltr4LM}) && varargin{vap_fltr4LM}(1) >= 3,
        ...
        error(['HoughCircleTransform: ''fltr4LM_R'' has to be ', ...
...
if nargin > (1 + vap_multirad),
    ...
    prm_multirad = varargin{vap_multirad}(1);
        ...
        error(['HoughCircleTransform: ''multirad'' has to be ', ...
...
if nargin > (1 + vap_fltr4accum),
    if isnumeric(varargin{vap_fltr4accum}) && ...
        ...
        error(['HoughCircleTransform: ''fltr4accum'' has to be ', ...
...
for k = 1 : size(centers,1),
    ...
    % Build the sgn. curve
    SgnCvMat_dx = repmat( (SCvR_J0:SCvR_J1) - centers(k,1) , ...
        [SCvR_I1 - SCvR_I0 + 1 , 1] );
    SgnCvMat_dy = repmat( (SCvR_I0:SCvR_I1)' - centers(k,2) , ...
        [1 , SCvR_J1 - SCvR_J0 + 1] );
    ...
    if isempty(SgnCv_LMPos),
        ...
        radii(k) = SgnCv_LMPos(end);
        for i_radii = (length(SgnCv_LMPos) - 1) : -1 : 1,
            centers = [ centers; centers(k,:) ];
            radii = [ radii; SgnCv_LMPos(i_radii) ];
        end
    end
end
